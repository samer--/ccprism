:- module(ccprism, [ run_with_tables/2, run_tab_expl/2, run_sampling//2, ccstore/2
...
:- use_module(library/lazymath,     [ max/3, min/3, add/3, sub/3, mul/3, pow/3, exp/2, log_e/2, surp/2
...
accum_stats(Pred,Stats) :- 
   ...
   maplist(ccprism:pmap_collate(right,=(0),C1),SWs,Stats).
...
make_tree_sampler(G, ccprism:sample_goal(P,IG)) :- graph_inside(G, P, IG).
